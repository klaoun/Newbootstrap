{combine_css path="themes/bnewootstrap_darkroom/admin/css/admin.css"}
{combine_script id='common' load='footer' path='admin/themes/default/js/common.js'}
<div class="titrePage">
    <h2>New Bootstrap Darkroom {$TABSHEET_TITLE}</h2>
</div>
    ...
    <div id="configContent">
      <div id="appearance" class="tab-content current">
        <fieldset class="mainConf">
            <legend>{'New Bootstrap theme'|@translate}</legend>
            <ul>
                <li>
                    <label labelfor="bootstrap_theme">{'Color theme'|@translate}</label>
                    <select name="bootstrap_theme">
                        <option value="bootstrap-darkroom"{if $theme_config->bootstrap_theme == 'newbootstrap-darkroom'} selected="selected"{/if}>New Bootstrap Darkroom</option>
...
function getBootswatchThemes() {
    $.getJSON("https://bootswatch.com/api/3.json", function (data) {
      ...
      preview.html('<img src="themes/newbootstrap_darkroom/components/bootswatch/' + select_bootswatch.val() + '/thumbnail.png" width="50%" style="padding: 10px 0;"/>');
      preview.show();
...
$(document).ready(function() {
  if ($('select[name=bootstrap_theme]').val() === 'bootswatch') {
    ...
    preview.html('<img src="themes/newbootstrap_darkroom/admin/img/' + $('select[name=bootstrap_theme]').val() + '.png" style="padding: 10px 0;"/>');
    preview.show();
  }
